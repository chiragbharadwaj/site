...
(function (window, document) {
    ...
    var init = function () {
        ...
        foundClocks = document.querySelectorAll('.cityclock');

        for (var i = 0, l = foundClocks.length; i < l; ++i) {
            var clockObj = {
                wrapper: foundClocks[i],
                canvas: null,
                ctx: null,
                state: 0,
                size: 70,
                radius: 35,
                utcOffset: parseInt(foundClocks[i].getAttribute('data-timezone'), 10),
                localTime: null
            };
        ...
        day = new Date();
        utc = day.getTime() + (day.getTimezoneOffset() * 60000);
        tick();
    };
    ...
    var initClock = function (clock) {
        ...
        clock.canvas.height = clock.size;
        clock.canvas.width = clock.size;

        clock.wrapper.insertBefore(clock.canvas, clock.wrapper.firstChild);
    };
    ...
    var tick = function () {
        ...
        for (var i = 0, l = clocks.length; i < l; ++i) {
            clock = clocks[i];
            clock.localTime = new Date(utc + (3600000 * clock.utcOffset));
            renderClock(clock);
        }
    ...
    var renderClock = function (clock) {
        clock.ctx.clearRect(0, 0, clock.size, clock.size);

        clock.ctx.beginPath();
        clock.ctx.arc(clock.radius, clock.radius, clock.radius, 0, Math.PI * 2, true);
        clock.ctx.fillStyle = '#0084BA';
        clock.ctx.closePath();
        clock.ctx.fill();
        ...
        clock.ctx.save();
        clock.ctx.rotate((clock.localTime.getHours() + (clock.localTime.getMinutes() / 60)) * (Math.PI / 6));
        clock.ctx.beginPath();
        clock.ctx.moveTo(0, 0);
        clock.ctx.lineTo(0, -0.5 * clock.radius);
        clock.ctx.strokeStyle = '#FFF';
        clock.ctx.lineWidth = 1.1;
        clock.ctx.closePath();
        clock.ctx.stroke();
        clock.ctx.restore();

        clock.ctx.save();
        clock.ctx.rotate((clock.localTime.getMinutes() + (clock.localTime.getSeconds() / 60)) * (Math.PI / 30));
        clock.ctx.beginPath();
        clock.ctx.moveTo(0, 0);
        clock.ctx.lineTo(0, -0.7 * clock.radius);
        clock.ctx.strokeStyle = '#FFF';
        clock.ctx.lineWidth = 1.1;
        clock.ctx.closePath();
        clock.ctx.stroke();
...
(function (window) {
    ...
    var url = 'https://api.lever.co/v0/postings/madewithlove?mode=json';
    ...
    function createJobs(_data) {
        for (var i = 0; i < _data.length; i = i + 1) {
            ...
            var shortDescription = description.substring(0, 500);

            if (title === 'Internship') {
                $('#openings-intern').append('<li>' + '<a class="job-title" href="' + link + '">' + '<h1 class="meta">' + location + ' &dash; ' + commitment + ' &dash; ' + team + '</h1>' + '<p class="meta">' + shortDescription + '</p>' + '</a>' + '</li>');
            } else if (team === 'Development') {
                var shortDescription = description.substring(0, 250);
                if (shortDescription.length < description.length) {
                    shortDescription += '...';
                }

                $('#openings-develop').append('<li>' + '<a class="job-title" href="' + link + '">' + '<h1>' + title + '</h1>' + '<p class="meta">' + location + ' &dash; ' + commitment + '</p>' + '</a>' + '</li>');
            }
        }
    }
...
(function (window, document) {
    ...
    var init = function () {
        timelineElm = document.querySelector('.timeline');
    ...
    var supportTouch = function () {
        timelineElm.addEventListener('touchstart', setActivePanel, false);
        timelineElm.addEventListener('touchen', unsetActivePanel, false);
    };

    var setActivePanel = function (event) {
        console.log(event);
    };

    var unsetActivePanel = function (event) {
        console.log(event);
    };
...
var max = function (number, max) {
    return number > max ? max : number;
};

var min = function (number, min) {
...
var LiveMap = (function () {
    function LiveMap(el) {
        ...
        this.handleInitialVisits = this.handleInitialVisits.bind(this);
        this.handleRealtimeVisits = this.handleRealtimeVisits.bind(this);
        this.addVisit = this.addVisit.bind(this);
    }
    LiveMap.prototype.render = function () {
        var _this = this;
        this.instance = L.mapbox.map(this.el, 'bramdevries.jfij450e', {
            accessToken: 'pk.eyJ1IjoiYnJhbWRldnJpZXMiLCJhIjoicEswWXFXWSJ9.eCnxDol6jUKqOtMj7wITjA',
            touchZoom: false,
        ...
        $(window).on('resize', function () {
            return _this.setOriginalView;
        });
    };

    LiveMap.prototype.start = function () {
        $.get('/api/visits').then(this.handleInitialVisits);
    };

    LiveMap.prototype.setOriginalView = function () {
        this.smallScreen = $(window).width() < 640;
        this.originalCoordinate = [this.smallScreen ? 25.85 : 35.85, 15];
        this.originalZoom = this.smallScreen ? 2 : 3;

        this.instance.setView(this.originalCoordinate, this.originalZoom);
    };

    LiveMap.prototype.realtime = function () {
        $.get('/api/visits/realtime').then(this.handleRealtimeVisits);
    };

    LiveMap.prototype.handleInitialVisits = function (response) {
        this.visits.initial = response.visits;

        this.visits.initial.forEach(this.addVisit);

        this.realtime();
    };

    LiveMap.prototype.handleRealtimeVisits = function (response) {
        var _this = this;
        this.visits.realtime = response.visits;

        this.interval = (330 / response.visits.length) * 1000;

        this.loop = setInterval(function () {
            if (_this.visits.realtime.length > 0) {
                _this.addRealtimeVisit();
            } else {
    ...
    LiveMap.prototype.addVisit = function (visit) {
        var _this = this;
        var coordinates = L.latLng([visit.latitude, visit.longitude]);
        var radius = visit.realtime ? max((visit.visits * 8) / 2, 5) : visit.visits % 4.5;
        var marker = L.circleMarker(coordinates, {
            weight: 0,
            fillColor: '#cb1a38',
            fillOpacity: 0.7,
            className: visit.realtime ? 'realtime' : ''
        }).setRadius(radius);
        ...
        if (visit.realtime) {
            ...
            popup.setContent('New user in ' + visit.name);

            marker.bindPopup(popup);
            marker.openPopup();

            timeout = min(this.interval * (Math.random()), 5000);
        ...
        setTimeout(function () {
            marker.setStyle({ fillColor: '#889398' }).redraw();
            marker.closePopup();

            var popup = new L.Popup({
                className: 'hoverable'
            });
            popup.setContent(visit.name + ' - ' + visit.visits + ' users');
    ...
    LiveMap.prototype.addRealtimeVisit = function () {
        var visit = this.visits.realtime.shift();
        visit.realtime = true;
